...
DEFAULT_ACTOR = "itsm-servicedesk-triage-agent"
...
def assess_access(
    ...
    subject_ref: str,
    requested_role: str,
    current_entitlements: list[str] | None = None,
    actor: str = DEFAULT_ACTOR,
    tenant: str = "",
    settings: Settings | None = None,
) -> dict[str, Any]:
    """Assess an access request for segregation-of-duties conflicts and route it for approval.

    Resolves the requested role to its entitlements, checks the set the subject WOULD hold
    against the conflict matrix, derives the approval chain, and writes an already-redacted audit
    event. Every access grant is consequential, so the result ALWAYS routes to the human-review
    console (rule R8) and nothing is provisioned here.

    Args:
      request_ref: An opaque reference to this access request.
      subject_ref: An opaque, fictional reference to the person who would receive access.
      requested_role: The catalog role being requested.
      current_entitlements: Entitlements the subject already holds (the union is what is checked).
      actor: The verified identity this call is attributed to.
...
TOOL_FUNCTIONS = (triage_case, assess_access, verify_audit_trail)
