...
class AgentCard:
    ...
    provider: str = "itsm-servicedesk-triage"
...
SKILLS: tuple[AgentSkill, ...] = (
    ...
    AgentSkill(
        id="assess_access",
        name="Access-request assessment",
        description=(
            "Assess one access request for segregation-of-duties conflicts, resolve the role to "
            "its entitlements, derive the approval chain, and ROUTE every grant to human review "
            "(rule R8). The verdict is computed by pure stdlib code; nothing is provisioned."
        ),
    ),
    AgentSkill(
...
def _resolve_url(settings: Settings) -> str:
    ...
    return f"https://itsm-servicedesk-triage.{settings.region}.internal.example/a2a"
