"""FastAPI application for ITSM Service Desk Agent (H3).
...
``ITSMDESK_PROFILE`` is NO CHOICE rather than a silent ``local``. Every
...
``ITSMDESK_S2S_TOKEN`` is wrong in the most dangerous direction:
...
_TOKEN_ENV = "ITSMDESK_S2S_TOKEN"
_INSECURE_DEMO_ENV = "ITSMDESK_ALLOW_INSECURE_DEMO"
_CORS_ORIGINS_ENV = "ITSMDESK_CORS_ORIGINS"
_ALLOWED_CALLERS_ENV = "ITSMDESK_S2S_ALLOWED_CALLERS"
_AUDIENCE_ENV = "ITSMDESK_S2S_AUDIENCE"
_API_HOST_ENV = "ITSMDESK_API_HOST"
...
def require_service_caller(request: Request) -> None:
    ...
    stays OPEN when ``ITSMDESK_S2S_TOKEN`` is unset (loopback dev with zero
    secrets), so an UNSET ``ITSMDESK_PROFILE`` must never be allowed to
    ...
    ``ITSMDESK_ALLOW_INSECURE_DEMO=1``.
    """
    if not _request_choice(request).service_auth_configured:
        raise HTTPException(
            ...
            detail=(
                ...
                "ITSMDESK_PROFILE is not set, so no authentication scheme "
                "has been chosen"
            ),
        )
...
_SERVICE_NAME = "itsm-servicedesk-triage"
...
_APP_TITLE = "ITSM Service Desk Agent"
...
# pill names is what answered, never what configuration says would. This service binds no model
# port: its engine is deterministic and notes nothing, so it sends neither header and the pill
# keeps showing the configured `generator_model` (`no-model`).
...
#
# Note what is NOT in this expression: ITSMDESK_S2S_TOKEN. A service
...
def access(
    ...
    result = service.assess(
        AccessRequest(
            ...
            current_entitlements=tuple(request.current_entitlements),
        ),
        actor=principal.actor,
    )
    routing = RecordingReviewRouter(container.review_router)
