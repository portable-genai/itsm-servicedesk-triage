...
(``ITSMDESK_REVIEW_ROUTING=off``), the container binds :class:`DisabledReviewRouter` instead of
...
than failing an already-scored, already-audited triage or access decision, but it is never
...
class RecordingReviewRouter:
    ...
    def route(
        ...
        if not getattr(self._inner, "enabled", True):
            ...
            return ""
        try:
            reference = self._inner.route(result, maker=maker, tenant=tenant, action=action)  # type: ignore[attr-defined]
