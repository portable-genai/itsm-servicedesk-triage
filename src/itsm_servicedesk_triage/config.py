...
One env var (``ITSMDESK_PROFILE``) selects the adapter family for every
...
_PROFILE_ENV = "ITSMDESK_PROFILE"
_SETTINGS_ENV = "ITSMDESK_SETTINGS"
...
#: What ``generator_model`` reports, and what the ``local`` stub notes as the model that answered,
#: when a model-shaped port is bound to the deterministic offline stub. One name for one stub, so
#: the pill reads the same before and after the first answer.
...
#: This service has NO generative port; the model pill says `no-model` rather than naming a stub.
_GENERATOR_PORT: str = ""
...
class ProfileChoice:
    ...
    The variable is ``ITSMDESK_PROFILE``, named once in
    :data:`_PROFILE_ENV`.
    ...
    ``os.environ.get("ITSMDESK_PROFILE", "local")``: that fallback reads an
...
def _read_settings_file(path: Path | None = None) -> dict[str, Any]:
    ...
    An EXPLICIT path (argument or ``ITSMDESK_SETTINGS``) that does not
...
REVIEW_ROUTING_ENV = "ITSMDESK_REVIEW_ROUTING"
...
class Settings:
    ...
    def generator_model(self) -> str:
        ...
        calls: under ``gcp`` the one its call passes, never a model a flag could swap in.
