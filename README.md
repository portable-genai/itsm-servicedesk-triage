...
itsm_servicedesk_triage triage "Acme (FICTIONAL)" "urgent data breach"
itsm_servicedesk_triage access REQ-1 person-0001 payments_maker --current payment.approve
```

The two verticals share the domain, the audit trail and the R8 routing: `triage` scores an inbound
ticket into a deterministic queue and severity from the taxonomy pack, and `access` checks an
access request against the segregation-of-duties matrix. A triage result escalates only when its
band is consequential; an access decision is consequential by construction, so it ALWAYS routes to
a human and nothing is provisioned in-process.
...
One env var, `ITSMDESK_PROFILE`, selects the adapter family:
...
`ITSMDESK_ALLOW_INSECURE_DEMO=1` says otherwise. The guard reads the identity
BINDING to decide that, never a service credential: setting
`ITSMDESK_S2S_TOKEN` closes the S2S routes and does not open anything else.
See `docs/runbook.md`.
...
**Name the profile.** `ITSMDESK_PROFILE` has no default. Leaving it unset is
