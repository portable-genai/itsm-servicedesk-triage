...
|---|---|---|
| Human review and maker-checker console | **Hrz7** `human-review-console` | **Yes.** `ports/review_router.py` with an adapter in every profile, on the shared `review-kit` (a hard dependency, not an extra). `review_url` comes from `HRZ_HUMAN_REVIEW_URL`; the managed router REFUSES when no console is configured rather than swallowing an escalation (rule R8, Covered). |
