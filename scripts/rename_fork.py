...
Rewrites the python package name, the console-script name, the ``ITSMDESK`` env-var prefix, the
Terraform resource-id stem and the distribution / git id across the tree, so a fork does not
...
Markdown prose alone (pass ``--include-docs`` to sweep it too).
    ...
    # Apply it:
    python scripts/rename_fork.py --package acme_servicedesk --cli acme-desk \
        --env-prefix ACME --resource acme-desk --yes
...
# --------------------------------------------------------------------------------------- #
_OLD_PACKAGE = "itsm_servicedesk_triage"
_OLD_CLI = "itsm_servicedesk_triage"
_OLD_ENV_STEM = "ITSMDESK"
_OLD_RESOURCE = "h3-svc"
_OLD_DIST = "itsm-servicedesk-triage"
...
def _rules(args: argparse.Namespace) -> list[tuple[str, str, str]]:
    ...
    return [
        ...
        # The env-var prefix. Matched WITHOUT the trailing underscore so both ITSMDESK_PROFILE
...
def main() -> int:
    ...
    parser.add_argument("--cli", required=True, help="new console-script name, e.g. acme-desk")
    parser.add_argument("--env-prefix", required=True, help="new env-var prefix stem, e.g. ACME")
    parser.add_argument(
        "--resource", required=True, help="new Terraform name_prefix stem, e.g. acme-desk"
    )
    parser.add_argument(
