...
| `rename_fork.py` | Not a demo script but a one-shot FORK tool, listed here because this directory is the index of everything in `scripts/`. It rebrands the repo for an adopter in one pass: the python package, the console-script name, the `ITSMDESK` env prefix, the Terraform `name_prefix` stem and the distribution id. Offline and stdlib-only like everything else here, it prints a plan and writes nothing without `--yes`. See `docs/ADOPTING.md`. |
