...
from itsm_servicedesk_triage.domain import (
    kernel,
    models,
)
...
SERVICE_NAME = "ITSM Service Desk Agent"
CATALOG_ID = "H3"
REPOSITORY = "itsm-servicedesk-triage"
...
class DemoRun:
    ...
    def _step_redaction(self) -> Produced:
        ...
        stored_citations = json.dumps(record.get("citations", []), sort_keys=True, default=str)
        # Scan the record's CONTENT fields, not the summary alone. The citations live in the same
        # immutable row, and a claim that read one field is exactly how a masked summary got to
        # certify a record whose citation still carried the identifier. `actor` stays out of the
        # scan on purpose: it is the verified principal and an address by design.
        leaked = PLANTED_NRIC in recorded or PLANTED_NRIC in stored_citations
        panels.append(
            Panel(
                ...
                rows=(
                    ...
                    Row("Stored citations", stored_citations),
                ),
                note=(
