...
this service has: ``ITSMDESK_REVIEW_ROUTING`` is read in three states; off binds a disabled router
and says so at startup; on under the managed profile refuses to boot without a console; and the API
(triage and access), the agent tools and the CLI report ``review_routing`` rather than failing an
...
from itsm_servicedesk_triage.adapters.controls import (
    DisabledReviewRouter,
    RecordingReviewRouter,
    ReviewRouting,
)
from itsm_servicedesk_triage.agent import tools
from itsm_servicedesk_triage.api import app as api_module
from itsm_servicedesk_triage.api.app import app
from itsm_servicedesk_triage.cli.main import main as cli_main
...
from itsm_servicedesk_triage.domain.models import TriageInput, TriageResult
from itsm_servicedesk_triage.domain.triage_service import TriageService
from itsm_servicedesk_triage.envread import ConfiguredEmptyError
...
_LOCAL_ROUTE = "itsm_servicedesk_triage.adapters.local.review_router.LocalReviewRouter.route"
...
def _gcp(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "itsm_servicedesk_triage.config.resolve_profile",
...
def test_the_off_posture_is_logged_once_however_many_containers(
    ...
    settings = Settings(profile="local", controls=ControlSwitches(review_routing=False))
    with caplog.at_level(logging.WARNING, logger="itsm_servicedesk_triage.config"):
        for _ in range(3):
...
class _Accepting:
    def route(
        self, result: TriageResult, *, maker: str, tenant: str = "", action: str = "review"
    ) -> str:
        return "review-1"
...
def test_a_failed_hand_off_is_reported_and_logged_never_raised(
    ...
    with caplog.at_level(logging.WARNING, logger="itsm_servicedesk_triage.adapters.controls"):
...
# --------------------------------------------------------------------------- #
# The access decision always hands off, on every surface
# --------------------------------------------------------------------------- #
_ACCESS = {
    "request_ref": "REQ-9",
...
def test_the_access_route_reports_routed_off_and_failed(monkeypatch: pytest.MonkeyPatch) -> None:
    assert _post("/v1/access", _ACCESS)["review_routing"] == "routed"

    monkeypatch.setattr(_LOCAL_ROUTE, _Refusing.route)
    failed = _post("/v1/access", _ACCESS)
    assert failed["review_routing"] == "failed"
    assert failed["review_ref"] == ""

    monkeypatch.setenv(REVIEW_ROUTING_ENV, "off")
    api_module._container.cache_clear()
    off = _post("/v1/access", _ACCESS)
    assert off["review_routing"] == "off"
    assert off["review_ref"] == ""


def test_the_access_tool_and_cli_report_the_hand_off(
    monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    tool_args = {k: v for k, v in _ACCESS.items() if k != "current_entitlements"}
    assert tools.assess_access(**tool_args)["review_routing"] == "routed"
    monkeypatch.setattr(_LOCAL_ROUTE, _Refusing.route)
    assert cli_main(["access", "REQ-9", "person-0009", "payments_maker"]) == 0
