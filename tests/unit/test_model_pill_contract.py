...
request; until one has, that pill names the configured model and says, in its title, WHERE the
process runs (owner decision, 2026-09-23, replacing the 2026-08-30 full-width banner). Both
come from ``/healthz`` because the browser cannot know either: a console that read its runtime
from ``window.location`` would be right until the day the deployment served through a proxy,
...
from itsm_servicedesk_triage.config import Settings
...
def test_the_health_contract_carries_both_halves() -> None:
    ...
    """
    from itsm_servicedesk_triage.api.schemas import HealthResponse
...
def test_not_implemented_is_claimed_only_by_an_adapter_that_never_calls_a_model() -> None:
    ...
    if settings.generator_model != "managed-not-implemented":
        return
    from itsm_servicedesk_triage.config import _GENERATOR_PORT
