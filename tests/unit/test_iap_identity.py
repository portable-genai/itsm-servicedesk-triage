...
from itsm_servicedesk_triage.adapters.gcp.identity import (
    _AUDIENCE_ENV,
...
from itsm_servicedesk_triage.config import Settings, _read_settings_file
from itsm_servicedesk_triage.ports.identity import (
    VERIFIED,
...
_API_MODULE = "itsm_servicedesk_triage.api.app"
...
def test_an_emptied_audience_is_refused_by_the_settings_loader(
    ...
    The `iap_audience: ${...:-}` line is a documented default with a variable allowed to override
    it, so it obeys the same rule as `setting_or_default`: an operator who emptied the variable
    expressed an intent that names nothing, and it must not silently become "no audience
    configured". The refusal happens at load, naming the variable.
    """
    path = tmp_path / "settings.yaml"
    path.write_text(_REBOUND_SETTINGS + "\n", encoding="utf-8")
