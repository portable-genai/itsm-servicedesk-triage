...
ITSM Service Desk Agent (H3): Deterministic IT service-desk agent: model-advised triage and access provisioning.
Rendered from `hex-service-template`, so it starts at reference parity rather than converging
toward it. Package `itsm_servicedesk_triage`, environment prefix
`ITSMDESK`, region `asia-southeast1`.
...
- `src/itsm_servicedesk_triage/domain/` is PURE stdlib. No web framework, no cloud SDK,
...
- **The exposure guard is derived from the IDENTITY BINDING, and from nothing else.** An end-user
  ...
  `ITSMDESK_S2S_TOKEN` may never enter that decision: it authenticates a
...
- **The one adapter that declares `VERIFIED` must EARN it, and it is the one adapter that may not
  ...
  configured `ITSMDESK_IAP_AUDIENCE`, three-state: unset or emptied REFUSES,
...
- **One profile read, three states, refused at import.** `config.PROFILE_CHOICE` resolves once at
  ...
  `ITSMDESK_PROFILE` (mentioning it in a refusal message is wanted, reading
